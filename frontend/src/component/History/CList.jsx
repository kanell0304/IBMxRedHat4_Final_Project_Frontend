export default function CList({ items, loading, error, onDelete, formatDate, onSelect }) {
  if (error) {
        ...
        >
          ...
          >
            <div className="flex items-center gap-2">
              <span className="text-sm font-semibold text-gray-900">대화 분석</span>
            </div>
            <p className="text-xs text-gray-400">{formatDate(item.created_at)}</p>
          </button>
          {onDelete && (
            <button
              onClick={() => onDelete(item.c_id)}
