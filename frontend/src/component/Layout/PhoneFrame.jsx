        ...
        <div className="relative h-full w-full bg-white overflow-hidden">
          <div className="relative h-full w-full overflow-y-auto scrollbar-hide">
            <header className="sticky top-0 z-20 bg-white/95 backdrop-blur">
              <div
                className={`relative px-4 pt-1.5 pb-1.5 ${
                  headerContent ? 'h-[100px]' : showTitleRow ? 'h-[76px]' : 'h-[52px]'
                }`}
              >
                <div className="absolute top-[10px] left-0 right-0 px-4 flex items-center justify-between text-[13px] text-gray-900 font-semibold">
                ...
                <div className="pointer-events-none absolute left-1/2 -translate-x-1/2 top-[8px] h-7 w-28 bg-black rounded-full shadow-[0_2px_10px_rgba(0,0,0,0.35)]" />
                {headerContent ? (
                  <div className="absolute left-0 right-0 -bottom-3 px-4 pb-1 pt-1">
                    {headerContent}
                  </div>
                ) : showTitleRow ? (
                  <div className="absolute left-0 right-0 bottom-0 flex items-center h-11 px-4 border-b border-gray-200">
                    <button
                      type="button"
                      onClick={handleBack}
