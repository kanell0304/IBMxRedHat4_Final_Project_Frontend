...
export default function CommunicationList() {
  ...
  const formatDate = (d) => new Date(d).toLocaleString('ko-KR', { year: 'numeric', month: '2-digit', day: '2-digit', hour: '2-digit', minute: '2-digit' });
  ...
  return (
    <PhoneFrame title="대화 분석">
      <div className="space-y-5">
        ...
        <button onClick={() => nav('/communication/info')}
        ...
        {list.length === 0 ? (
          <div className="rounded-3xl bg-white shadow-sm p-12 text-center space-y-4 border border-slate-100">
            ...
            <h2 className="text-lg font-bold text-gray-900">아직 분석한 대화가 없어요</h2>
            <p className="text-sm text-gray-600">첫 대화 분석을 시작해보세요!</p>
            <button onClick={() => nav('/communication/info')}
                ...
                <button key={c.c_id}
                  ...
                  <div className="flex-1 text-left space-y-1">
                    <div className="flex items-center gap-2">
                      <span className="text-sm font-bold text-gray-900">대화 #{c.c_id}</span>
                      <span className={`text-[11px] font-semibold rounded-full px-2.5 py-1 ${cfg.color}`}>{cfg.text}</span>
                    </div>
                    <p className="text-[13px] text-gray-600">{formatDate(c.created_at)}</p>
                  </div>
